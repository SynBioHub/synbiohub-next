...
export default class SBOLFetcherICE {
    ...
    async fetchSBOLObjectRecursive(remoteConfig, sbol, type, uri) {
        ...
        if(displayId === remoteConfig.rootCollection.displayId) {
            ...
            let rootFolders = await ice.getRootFolders(remoteConfig)
    ...
    if(displayId === 'available') {
        ...
        if(displayId.endsWith(remoteConfig.sequenceSuffix)) {
            ...
            let part = await ice.getPart(remoteConfig, partDisplayId)
                
            let iceSbol = await ice.getSequence(remoteConfig, part.id)
        ...
        let part = await ice.getPart(remoteConfig, displayId)
        ...
        async function availableFolderToCollection(folderId) {
            ...
            let entries = await ice.getRootFolderEntries(remoteConfig)
            ...
            collection.wasDerivedFrom = remoteConfig.url + '/folders/available'

            let componentDefinitions = await Promise.all(entries.map((entry) => {
    ...
    private async partToComponentDefinition(part) {

        if(part.hasSequence) {

            let seq = await ice.getSequence(remoteConfig, part.id)
        ...
        function createSBOL(iceSbol) {
            ...
            componentDefinition.addUriAnnotation('http://wiki.synbiohub.org/wiki/Terms/ice#entry', remoteConfig.url + '/entry/' + part.partId)
