...
export default async function(req, res) {
	...
	if (!benchlingRemote) {
		if (req.method === 'GET') {
			...
			if (req.user && req.user.isMember) {
				...
				Object.keys(config.get('remotes')).filter(function (e) {
					return config.get('remotes')[e].type === 'benchling' &&
						config.get('remotes')[e].public
				}).forEach((remote) => {
					benchlingRemotes.push(config.get('remotes')[remote])
				})
			}
	...
	let result2 = await benchling.createSequence(remoteConfig, bSeq)

	console.log(JSON.stringify(result2))
	res.redirect('/public/' + remoteConfig.id + '/' + result2.id /*remoteConfig.folderPrefix + bSeq.folder*/ + '/current')
}
	...
	cd.sequenceAnnotations.forEach((sa) => {
		if (sa.component && sa.component != '') {
			if (sa.component.definition.components.length > 0) {
				...
				let results = flattenSequenceAnnotations(sa.component.definition)
